...
ROOT = Path('.')
MODULO = ROOT / 'corponu-faccoes-corte-definitivo.js'
ABAS = ROOT / 'corponu-faccoes-tres-abas-saida.js'
ATUALIZADOR = ROOT / 'corponu-atualizador.js'
TESTE = ROOT / 'tests/e2e/faccoes-consolidado.spec.js'

for arquivo in [MODULO, ABAS, ATUALIZADOR, TESTE]:
...
def trocar_uma(texto, antigo, novo, descricao):
    if antigo not in texto:
        raise SystemExit(f'ERRO: não encontrei {descricao}.')
    if texto.count(antigo) != 1:
        raise SystemExit(f'ERRO: {descricao} apareceu {texto.count(antigo)} vezes.')
...
def trocar_regex(texto, padrao, novo, descricao):
    ...
    if total != 1:
        raise SystemExit(f'ERRO: substituição de {descricao} encontrou {total} ocorrências.')
    return resultado


texto = MODULO.read_text(encoding='utf-8')
texto = texto.replace('const VERSION = "2026-07-30-faccoes-corte-23";', 'const VERSION = "2026-08-21-lateral-alca-unificada-226";', 1)
...
marcador_helpers = '  const ehAdmin = () => norm(perfil?.tipo) === "ADMIN";\n'
...
texto = trocar_uma(texto, marcador_helpers, helpers, 'ponto de inclusão dos helpers de Lateral e Alça')
...
texto = trocar_uma(
    ...
    'título principal da área Corte'
)
texto = trocar_uma(
    texto,
    ...
    'botão Registrar saída da área Corte'
)
texto = trocar_uma(
    texto,
    ...
    'rótulo da aba interna Corte'
)

for antigo, novo, descricao in [
    ('<h3>Registrar saída de Corte</h3>', '<h3>Registrar saída de Lateral e Alça</h3>', 'título do modal de saída'),
    ('<h3 id="tituloChegadaCorte">Registrar chegada de Corte</h3>', '<h3 id="tituloChegadaCorte">Registrar chegada de Lateral e Alça</h3>', 'título do modal de chegada'),
    ('<h3>Selecionar movimentação em andamento</h3><p>Escolha qual OP retornou da facção.</p>', '<h3>Selecionar movimentação em andamento</h3><p>Escolha qual OP retornou de Lateral ou Alça.</p>', 'texto do seletor de chegada'),
]:
    texto = trocar_uma(texto, antigo, novo, descricao)
...
texto = trocar_uma(
    ...
    'contador da aba Lateral e Alça'
)
...
texto = trocar_uma(
    ...
    'evento da chegada manual de Lateral e Alça'
)
...
texto = trocar_regex(
    ...
    '',
    'MutationObserver global do módulo definitivo'
)

if 'new MutationObserver' in texto:
    raise SystemExit('ERRO: ainda existe MutationObserver no módulo definitivo de Lateral e Alça.')
if 'VALOR_FILTRO_ALCA' not in texto or 'movimentoPertenceLateralAlca' not in texto:
    raise SystemExit('ERRO: unificação de Lateral e Alça ficou incompleta.')
...
atualizador = '\n'.join(linha for linha in atualizador.split('\n') if not any(nome in linha for nome in linhas_remover))
if any(nome in atualizador for nome in linhas_remover):
    raise SystemExit('ERRO: atualizador ainda carrega remendos de Lateral e Alça.')
ATUALIZADOR.write_text(atualizador, encoding='utf-8')

teste = TESTE.read_text(encoding='utf-8')
teste = trocar_uma(
    teste,
    '    await expect(page.locator(\'#painelFaccoesCorte\')).toHaveCount(1);\n',
    ...
    'validação do painel consolidado no Playwright'
)
teste = trocar_uma(
    teste,
    "    expect(locais).not.toContain('corponu-lateral-observacao-opcional.js');\n",
    "    expect(locais).not.toContain('corponu-lateral-observacao-opcional.js');\n    expect(locais).not.toContain('corponu-faccoes-lateral-alca-integracao.js');\n    expect(locais).not.toContain('corponu-faccoes-label-lateral.js');\n",
    'lista de remendos proibidos no Playwright'
)
teste = trocar_uma(
    teste,
    "    expect(locais).toContain('corponu-faccoes-corte-definitivo.js');\n",
    "    expect(locais).toContain('corponu-faccoes-corte-definitivo.js');\n    expect(await page.evaluate(() => typeof window.__CORPONU_FACCOES_LATERAL_ALCA__)).toBe('undefined');\n",
    'marcador da integração antiga no Playwright'
)
TESTE.write_text(teste, encoding='utf-8')

print('Migração de Lateral e Alça preparada com sucesso.')
