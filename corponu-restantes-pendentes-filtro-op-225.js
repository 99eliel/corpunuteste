(() => {
  ...
  const VERSION = "2026-08-21-restantes-filtro-eventos-252";
  ...
  function injetarEstilo() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #${PAINEL_ID} .restantes-filtro-op-225{display:grid;grid-template-columns:minmax(220px,420px) auto minmax(120px,auto);gap:9px;align-items:end;padding:11px 16px;border-top:1px solid #fed7aa;border-bottom:1px solid #fed7aa;background:#fff7ed}
      #${PAINEL_ID} .restantes-filtro-op-225 label{display:grid;gap:5px;min-width:0;color:#7c2d12;font-size:11px;font-weight:900}
      #${INPUT_ID}{width:100%;min-height:39px;box-sizing:border-box;padding:8px 11px;border:1px solid #fdba74;border-radius:9px;background:#fff;color:#0f172a;font-size:12px;font-weight:700;line-height:1.3;outline:none}
      #${INPUT_ID}:focus{border-color:#ea580c;box-shadow:0 0 0 3px rgba(234,88,12,.12)}
      #${LIMPAR_ID}{min-height:39px;padding:8px 13px;border:1px solid #fdba74;border-radius:9px;background:#fff;color:#9a3412;font-size:11px;font-weight:900;cursor:pointer}
      #${LIMPAR_ID}:disabled{opacity:.5;cursor:default}
      #${STATUS_ID}{min-height:39px;display:flex;align-items:center;justify-content:center;box-sizing:border-box;padding:8px 11px;border-radius:9px;background:#ffedd5;color:#9a3412;font-size:11px;font-weight:900;white-space:nowrap}
      #${VAZIO_ID}{padding:11px 16px;border-bottom:1px solid #fed7aa;background:#fff;color:#9a3412;font-size:12px;font-weight:800;text-align:center}
      #${VAZIO_ID}.hidden{display:none!important}
      @media(max-width:720px){#${PAINEL_ID} .restantes-filtro-op-225{grid-template-columns:1fr auto}#${STATUS_ID}{grid-column:1/-1;justify-content:flex-start}}
    `;
  ...
  function opDaLinha(linha) {
    ...
    return normalizarOP(forte?.textContent || linha.cells?.[0]?.textContent || "");
  }

  function linhasDeDados(tbody) {
    return [...tbody.querySelectorAll(":scope > tr")].filter(linha =>
      linha instanceof HTMLTableRowElement && !linha.querySelector("td.empty") && linha.cells.length >= 2
    );
  }
  ...
  function atualizarVazio(busca, total, visiveis) {
    ...
    if (busca && total > 0 && visiveis === 0) {
      vazio.textContent = `Nenhum restante pendente encontrado para a OP ${texto(document.getElementById(INPUT_ID)?.value)}.`;
  ...
  function aplicarFiltro() {
    ...
    try {
      ...
      linhas.forEach(linha => {
        const mostrar = !busca || opDaLinha(linha).includes(busca);
  ...
  function observarLista(tbody) {
    if (!(tbody instanceof HTMLTableSectionElement) || listaObservada === tbody) return;
  ...
  function criarInterface(painel, tbody) {
    ...
    filtro.innerHTML = `
      <label for="${INPUT_ID}">Filtrar por OP<input id="${INPUT_ID}" type="search" inputmode="numeric" autocomplete="off" placeholder="Digite o número da OP..." /></label>
  ...
  function aguardarPainel() {
    ...
    if (!pagina || observerPainel) return;

    observerPainel = new MutationObserver(() => {
      queueMicrotask(() => {
        if (!instalarNoPainel()) return;
        observerPainel?.disconnect();
        observerPainel = null;
      });
    });
    observerPainel.observe(pagina, { childList: true, subtree: true });
  }
  ...
  function instalarEventos() {
    document.addEventListener("click", event => {
      ...
      if (alvo.closest("#btnRestantesPagamento")) {
        queueMicrotask(aguardarPainel);
        requestAnimationFrame(aguardarPainel);
      }
      if (alvo.closest("#btnAtualizarRestantesPagamento")) queueMicrotask(aplicarFiltro);
    }, true);
  }
  ...
  function iniciar() {
    instalarEventos();
    aguardarPainel();
    console.info(`[CorpoNu] Filtro de OP em Restantes orientado a eventos: ${VERSION}`);
  }

  window.CorpoNuFiltroRestantes = { versao: VERSION, aplicar: aplicarFiltro };
